...
class SidecarOrchestrator:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("✅ Legal Review Sidecar initialized")
        except Exception as e:
            logger.error(f"❌ Failed to initialize Legal Review Sidecar: {e}")
            self.legal_agent = None
            
        try:
            self.threat_agent = ThreatModelingSidecar()
            logger.info("✅ Threat Modeling Sidecar initialized")
        except Exception as e:
            logger.error(f"❌ Failed to initialize Threat Modeling Sidecar: {e}")
...
if __name__ == "__main__":
    ...
    async def test_sidecars():
        ...
        print(f"Main assessment completed - Confidence: {result.confidence:.2f}")
